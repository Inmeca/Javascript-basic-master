...
{
    function ObtenerCorridasWeb (descripcionServicio:string, empresa:string = "AEF") {
        var corridas :[object] = [{}];
        var band = false;
...
class User implements IUser{
    ...
    calcularEdad(dia:string, mes:string, anio:string) {
        const dateNow = new Date();
        var anioNumber :number = + anio;
        var mesNumber :number = + mes;
        var diaNumber :number = + dia;
        var anios : number ;
    ...
    obtenerUsuario(correo: string):object{
        var id : string;
        var datosUsuario : string[];
        id = "0";

        directory.forEach(d => {
            if(d.email == correo){ id = d.id;  }
        });
